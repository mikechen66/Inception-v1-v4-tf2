...
If the files of nception_v3_weights are hard to download during the runtime due to the hindrince 
...
Please make the following command in the Linux Terminal and get the related model summary. 

$ python inception_v3_model.py
...
from keras.layers import Conv2D, Dense, Input, Activation, MaxPooling2D, BatchNormalization, \
...
def Inception_V3(include_top=True, weights='imagenet', input_tensor=None, 
    ...
    Instantiates the Inception v3 architecture. Optionally loads weights 
    pre-trained on ImageNet. Note that when using TensorFlow, for best 
    performance you should set `image_data_format="channels_last"` in your 
    Keras config at ~/.keras/keras.json.

    The model and the weights are compatible with both TensorFlow and Theano. 
    The data format convention used by the model is the one specified in your 
    Keras config file. Note that the default input image size for this model 
    is 299x299.

    Arguments:
        include_top: whether to include the fully-connected layer at the top
            of the network.
        weights: one of `None` (random initialization) or "imagenet" (pre-
        ...
        input_shape: optional shape tuple, only to be specified if `include_top` 
            ...
            and height should be no smaller than 139, e.g. `(150, 150, 3)` would 
            be one valid value.
    ...
    if weights == 'imagenet':
        ...
        if include_top:
            ...
            weights_path = 'home/mike/keras_dnn_models/inception_v3_weights_tf_dim_ordering_tf_kernels_notop.h5'
