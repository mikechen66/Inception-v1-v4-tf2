...
class BasicConv2D(layers.Layer):
    def __init__(self, kernels, kernel_size=(3,3), strides=1, padding='valid'):
        ...
        self.conv = layers.Conv2D(kernels, kernel_size, strides=strides, padding=padding, use_bias=False)
...
class InceptionA(layers.Layer):
    ...
    def call(self, x, training=False):
        ...
        return tf.concat(outputs, axis=-1)  # CHECK AXIS
...
class InceptionV3(Model):
    def __init__(self):
        ...
        self.conv6 = Sequential([InceptionE(),InceptionE()])
...
if __name__ == '__main__':
    ...
    model = InceptionV3().call(input_shape)
