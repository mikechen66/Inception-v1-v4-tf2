...
3x3 kernel_size. Due to the reason, Cholett's weights are not suitable for the script. So users need 
to generate the Inception 3 weights for a further usage. If users want to run the model, please run 
the the command as follows.
...
from tensorflow.keras.layers import Input, Conv2D, Dropout, Dense, Flatten, Activation, \
